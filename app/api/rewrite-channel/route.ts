...
async function logUsageToSupabase(
  ...
  try {
    ...
    if (error) {
      ...
      console.error('❌ Failed to log rewrite-channel usage:', error);
    } else {
      logger.log('📊 Rewrite-channel usage logged:', {
    ...
    console.error('❌ Exception logging rewrite-channel usage:', err);
  }
}
  ...
  try {
    ...
    if (userId) {
      ...
      logUsageToSupabase(
        ...
        console.error('❌ Unexpected error in rewrite-channel usage logging:', err);
      });
    } else {
      logger.log('⚠️ No user ID found, skipping usage logging for rewrite-channel');
    }
