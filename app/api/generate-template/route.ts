...
async function logUsageToSupabase(
  ...
  try {
    ...
    if (error) {
      ...
      console.error('❌ Failed to log generate-template usage:', error);
    } else {
      logger.log('📊 Generate-template usage logged:', {
    ...
    console.error('❌ Exception logging generate-template usage:', err);
  }
}
...
REMEMBER: Output ONLY HTML. No markdown, no preamble, no explanation. Just the HTML content.`,
    ...
    if (userId) {
      ...
      logUsageToSupabase(
        ...
        console.error('❌ Unexpected error in generate-template usage logging:', err);
      });
    } else {
      logger.log('⚠️ No user ID found, skipping usage logging for generate-template');
    }
