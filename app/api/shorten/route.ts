...
async function logUsageToSupabase(
  ...
  try {
    ...
    if (error) {
      ...
      console.error('❌ Failed to log shorten usage:', error);
    } else {
      logger.log('📊 Shorten usage logged:', {
    ...
    console.error('❌ Exception logging shorten usage:', err);
  }
}
  ...
  try {
    ...
    if (userId) {
      ...
      logUsageToSupabase(
        ...
        console.error('❌ Unexpected error in shorten usage logging:', err);
      });
    } else {
      logger.log('⚠️ No user ID found, skipping usage logging for shorten');
    }
